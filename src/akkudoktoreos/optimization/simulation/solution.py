"""Solver-agnostic simulation solution."""
...
from akkudoktoreos.optimization.simulation.parameters import SimulationParametersBaseModel
from akkudoktoreos.optimization.optimization import OptimizationSolution
from akkudoktoreos.utils.datetimeutil import to_datetime, to_duration
from akkudoktoreos.utils.utils import NumpyEncoder


class DeviceOptimizeResult(SimulationParametersBaseModel):
    device_id: str = Field(
...
class SimulationResult(SimulationParametersBaseModel):
...
class SimulationSolution(ConfigMixin, SimulationParametersBaseModel):
    ...
    **Note**: The first value of "load_wh_per_hour", "grid_feed_in_wh_per_hour",
    and "grid_consumption_wh_per_hour", will be set to null in the JSON output
    and represented as NaN or None in the corresponding classes' data returns.
    This approach is adopted to ensure that the current hour's processing remains unchanged.
    """
    ...
    # --- Backward-compatibility: deprecated German field aliases ---
    ...
    @field_validator("ev_obj", mode="before")
    @classmethod
    def convert_eauto(cls, field: Any) -> Any:
        """Convert Battery instance to ElectricVehicleResult."""
